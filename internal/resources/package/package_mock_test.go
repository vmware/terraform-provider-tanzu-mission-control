...
func (testConfig *testAcceptanceConfig) setupHTTPMocks(t *testing.T) {
	...
	getResponse := &pakageclustermodel.VmwareTanzuManageV1alpha1ClusterNamespaceTanzupackageMetadataGetPackageResponse{
	...
	getPkgEndpoint := (helper.ConstructRequestURL(https, endpoint, clAPIVersionAndGroup, testConfig.ScopeHelperResources.Cluster.Name, apiSubGroup, globalRepoNamespace, apiKind, pkgMetadataName, "packages", testConfig.PkgName)).String()
