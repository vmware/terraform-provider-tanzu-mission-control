...
func TestFlattenCustomIngress(t *testing.T) {
	t.Parallel()

	cases := []struct {
		description string
		input       *policyrecipenetworkmodel.V1alpha1CommonPolicySpecNetworkV1CustomIngress
		expected    []interface{}
	}{
		{
			description: "check for nil custom-ingress recipe network policy ",
			input:       nil,
			expected:    nil,
		},
		{
			description: "normal scenario with valid custom-ingress recipe network policy ",
			input: &policyrecipenetworkmodel.V1alpha1CommonPolicySpecNetworkV1CustomIngress{
	...
	for _, each := range cases {
		...
		t.Run(test.description, func(t *testing.T) {
			actual := FlattenCustomIngress(test.input)
