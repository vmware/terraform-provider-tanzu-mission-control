//go:build iampolicytemplate
// +build iampolicytemplate
...
func TestAcceptanceIAMPolicyTemplateResource(t *testing.T) {
	err := context.Setup()
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: tfResourceConfigBuilder.GetSlimIAMPolicyTemplateConfig(),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(IAMPolicyTemplateResourceFullName, "name", IAMPolicyTemplateName),
					verifyTanzuKubernetesClusterResource(provider, IAMPolicyTemplateResourceFullName, IAMPolicyTemplateName),
				),
			},
			{
				Config: tfResourceConfigBuilder.GetFullIAMPolicyTemplateConfig(),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(IAMPolicyTemplateResourceFullName, "name", IAMPolicyTemplateName),
					verifyTanzuKubernetesClusterResource(provider, IAMPolicyTemplateResourceFullName, IAMPolicyTemplateName),
				),
...
func verifyTanzuKubernetesClusterResource(
	...
	return func(s *terraform.State) error {
		...
		fn := &iampolicytemplatemodels.VmwareTanzuManageV1alpha1PolicyTemplateFullName{
			Name: iamPolicyTemplateName,
		}

		resp, err := context.TMCConnection.IAMPolicyTemplateResourceService.IAMPolicyTemplateResourceServiceGet(fn)
