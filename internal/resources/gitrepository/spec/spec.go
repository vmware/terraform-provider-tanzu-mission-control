...
func expandRef(data []interface{}) (ref *gitrepositoryclustermodel.VmwareTanzuManageV1alpha1ClusterNamespaceFluxcdGitrepositoryReference) {
	...
	if v, ok := refData[branchKey]; ok {
		helper.SetPrimitiveValue(v, &ref.Branch, branchKey)
	}

	if v, ok := refData[tagKey]; ok {
		helper.SetPrimitiveValue(v, &ref.Tag, tagKey)
	}

	if v, ok := refData[semverKey]; ok {
		helper.SetPrimitiveValue(v, &ref.Semver, semverKey)
	}

	if v, ok := refData[commitKey]; ok {
		helper.SetPrimitiveValue(v, &ref.Commit, commitKey)
	}
